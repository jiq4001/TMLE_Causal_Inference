---
title: "TMLE Data Summary and Wragling-GI bleeding as Outcome"
author: "Jingjing Qi"
...
max_censor <- 12 #/months use large number for admistritive sencoring or desired max time frame   (max gib/death happened at 25month)
```
...
The data is right censored at 12 months after the date of discharge and summarized by time node of a 3 months duration.         
...
## Digoxin 12-month censoring / 3-month node  
...
alltp <- sapply(seq(1, 60, by = 4), function(x){
  rowSums(tableL[, x : (x+3)])
})
colnames(alltp) <- gsub("_1", "", colnames(tableL)[seq(1, 60, by = 4)])

alltpave <- sapply(seq(61, 88, by = 4), function(x){
  rowSums(tableL[, x : (x+3)])
})
colnames(alltpave) <- gsub("_1", "", colnames(tableL)[seq(61, 88, by = 4)])
...
surv_dat <- 
  ...
  merge.data.frame(ques2 %>%
                     select(id, month_to_outcome, month_to_gib, idx)%>%
                     mutate(month_to_outcome = ceiling(month_to_outcome / int_tp)), all.x = T)%>%
  group_by(id)%>%
  arrange(id, nodes)%>%
  filter(nodes <= max(event2nd$nodes))%>%
  fill(month_to_gib, .direction = "down")%>%
  mutate(ever_event = ifelse(!is.na(month_to_gib) & month_to_gib <= max_censor, 1, 0))%>%
  filter(month_to_outcome <= max_censor)%>%
  ...
  select(-idx, -id, -nodes, -month_to_gib)
...
```{r, echo=F}
load("../data/diox_12_3_B_tmle.RData")
...
## Ace/Arb 12-month censoring / 3-month node    
...
### The covariate-adjusted time-dependent Cox proportional hazards model   
```{r, echo=F}
load("../data/ace_arb_12_3_B_tmle.RData")
#summary(result.regList)
cuma <- summary(result.regList)$cmat["cum.A",]
```
...
| Unadjusted    | `r round(summary(fit)$coefficients[2], 2)`  | `r round(summary(fit)$conf.int[3], 2)`-`r round(summary(fit)$conf.int[4], 2)` |   `r round(summary(fit)$waldtest[3], 2)`  |
| Adjusted      | `r round(exp(cuma[1]), 2)`                  | `r round(exp(cuma[3]), 2)`-`r round(exp(cuma[4]), 2)`                         |   `r ifelse(round(cuma[5], 2) < 0.01, "<0.01", round(cuma[5], 2))`                                  |
## Pde5i 12-month censoring / 3-month node 
...
### The covariate-adjusted time-dependent Cox proportional hazards model   
```{r, echo=F}
load("../data/pde5i_12_3_B_tmle.RData")
#summary(result.regList)
cuma <- summary(result.regList)$cmat["cum.A",]
```
