...
ques2 %>%
  ...
  mutate(med_date_update = visit_date, # lag(visit_date, default = unique(discharge_date)), no use for now   
         ...
         visits_int = (days_to_visit - lag(days_to_visit, default = 0))/unit)-> med2
...
Med <- med3 %>%
  filter(month_to_visit <= min(max_censor, max(ques2$max_censor)))
Med  <- Med[, c(3, 60:90 )]%>%
  filter(nodes > 0) %>%   
...
gtsummary::tbl_summary(tableW, by = TRT)%>%
  bold_labels()%>%
  as_gt()
```
...
surv_dat <- 
  expand.grid(id = event2nd$id, 
            nodes = 1:(max(ques2$month_to_outcome)/int_tp)) %>% 
  merge.data.frame(ques2 %>%
                     select(id, month_to_outcome, month_to_gib_or_death, idx)%>%
                     mutate(month_to_outcome = pmin(ceiling(month_to_outcome / int_tp), max(event2nd$nodes)),
                            month_to_gib_or_death = ifelse(!is.na(month_to_gib_or_death),
                                                           pmin(ceiling(month_to_gib_or_death/int_tp), max(event2nd$nodes)), NA)),
                   all.x = T)%>%
  filter(nodes <= month_to_outcome)%>%
  group_by(id)%>%
  arrange(id, nodes)%>%
  fill(month_to_gib_or_death, .direction = "down")%>%
  mutate(ever_event = ifelse(!is.na(month_to_gib_or_death), 1, 0))%>%
  merge.data.frame(med3%>%select(idx, d_digoxin) %>%  #---------change ---------------  i_ace_arb   i_digoxin
                     group_by(idx) %>%
                     summarize(Digoxin = ifelse(sum(d_digoxin, na.rm = T) != 0, "Yes", "Never"))%>%
                     data.frame(), all.x = T, by = "idx")%>%
  merge.data.frame(med3%>%select(idx, nodes, d_digoxin) %>% 
                     group_by(idx, nodes)%>%
                     summarise(d_digoxin = sum(d_digoxin, na.rm = T)), all.x = T)%>%
  mutate(d_digoxin = replace_na(d_digoxin, 0),
         nodes_bl = nodes - 1,
         status = ifelse(!is.na(month_to_gib_or_death)& month_to_gib_or_death > nodes_bl & month_to_gib_or_death <= nodes, 1, 0))


fit <- survfit(Surv(month_to_outcome, ever_event) ~ Digoxin, data = surv_dat%>%
                         select(idx, month_to_outcome, ever_event, Digoxin)%>%
                         unique())
...
fit <- survival::coxph(Surv(month_to_outcome, ever_event) ~ Digoxin,
                       data = surv_dat%>%
                         select(idx, month_to_outcome, ever_event, Digoxin)%>%
                         unique())

fit_td <- survival::coxph(Surv(nodes_bl, nodes, status) ~ d_digoxin, 
                          data = surv_dat%>%
                            select(idx, nodes_bl, nodes, status, d_digoxin, month_to_gib_or_death, month_to_outcome)%>%
                            mutate(month_to_gib_or_death = ifelse(is.na(month_to_gib_or_death),
                                                                  month_to_outcome, month_to_gib_or_death),
                                   d_digoxin = ifelse(d_digoxin != 0, 1, 0))%>%
                            filter(nodes <= month_to_gib_or_death))
```
...
### The time-varing Cox proportional hazards model   

The HR of time-varing Cox PH model was `r round(summary(fit_td)$coefficients[2], 2)` (95% CI: `r round(summary(fit_td)$conf.int[3], 2)` - `r round(summary(fit_td)$conf.int[4], 2)`).    
...
The HR of marginal structural model was `r round(exp(cuma[1]), 2)` (95% CI: `r round(exp(cuma[3]), 2)` - `r round(exp(cuma[4]), 2)`).
...
| Time-Varing Cox-PH     | `r round(summary(fit_td)$coefficients[2], 2)`  | `r round(summary(fit_td)$conf.int[3], 2)`-`r round(summary(fit_td)$conf.int[4], 2)` |   `r round(summary(fit_td)$waldtest[3], 2)`  |
| Marginal Structural Model      | `r round(exp(cuma[1]), 2)`                  | `r round(exp(cuma[3]), 2)`-`r round(exp(cuma[4]), 2)`                         |   `r round(cuma[5], 2)`                                  |
...
surv_dat <- 
  ...
  fill(month_to_gib_or_death, .direction = "down")%>%
  mutate(ever_event = ifelse(!is.na(month_to_gib_or_death), 1, 0))%>%
  merge.data.frame(med3%>%select(idx, d_ace_arb) %>%  #---------change ---------------  i_ace_arb   i_digoxin
                     group_by(idx) %>%
                     summarize(Ace_or_Arb = ifelse(sum(d_ace_arb, na.rm = T) != 0, "Yes", "Never"))%>%
                     data.frame(), all.x = T, by = "idx")%>%
  merge.data.frame(med3%>%select(idx, nodes, d_ace_arb) %>% 
                     group_by(idx, nodes)%>%
                     summarise(d_ace_arb = sum(d_ace_arb, na.rm = T)), all.x = T)%>%
  mutate(d_ace_arb = replace_na(d_ace_arb, 0),
         nodes_bl = nodes - 1,
         status = ifelse(!is.na(month_to_gib_or_death)& month_to_gib_or_death > nodes_bl & month_to_gib_or_death <= nodes, 1, 0))


fit <- survfit(Surv(month_to_outcome, ever_event) ~ Ace_or_Arb, data = surv_dat%>%
                         select(idx, month_to_outcome, ever_event, Ace_or_Arb)%>%
                         unique())
ggsurvplot(fit, data = surv_dat, censor = T, palette = "jco", pval = F, pval.coord = c(12, 0.8))
#ace_arb <- survfit(Surv(month_to_outcome, ever_event) ~ ace_arb, data = surv_dat)

fit <- survival::coxph(Surv(month_to_outcome, ever_event) ~ Ace_or_Arb,
                       data = surv_dat%>%
                         select(idx, month_to_outcome, ever_event, Ace_or_Arb)%>%
                         unique())

fit_td <- survival::coxph(Surv(nodes_bl, nodes, status) ~ d_ace_arb, 
                          data = surv_dat%>%
                            select(idx, nodes_bl, nodes, status, d_ace_arb, month_to_gib_or_death, month_to_outcome)%>%
                            mutate(month_to_gib_or_death = ifelse(is.na(month_to_gib_or_death),
                                                                  month_to_outcome, month_to_gib_or_death),
                                   d_ace_arb = ifelse(d_ace_arb != 0, 1, 0))%>%
                            filter(nodes <= month_to_gib_or_death))
```
...
surv_dat <- 
  ...
  fill(month_to_gib_or_death, .direction = "down")%>%
  mutate(ever_event = ifelse(!is.na(month_to_gib_or_death), 1, 0))%>%
  merge.data.frame(med3%>%select(idx, d_pde5i) %>%  #---------change ---------------  i_ace_arb   i_digoxin
                     group_by(idx) %>%
                     summarize(Pde5i = ifelse(sum(d_pde5i, na.rm = T) != 0, "Yes", "Never"))%>%
                     data.frame(), all.x = T, by = "idx")%>%
  merge.data.frame(med3%>%select(idx, nodes, d_pde5i) %>% 
                     group_by(idx, nodes)%>%
                     summarise(d_pde5i = sum(d_pde5i, na.rm = T)), all.x = T)%>%
  mutate(d_pde5i = replace_na(d_pde5i, 0),
         nodes_bl = nodes - 1,
         status = ifelse(!is.na(month_to_gib_or_death)& month_to_gib_or_death > nodes_bl & month_to_gib_or_death <= nodes, 1, 0))


fit <- survfit(Surv(month_to_outcome, ever_event) ~ Pde5i, data = surv_dat%>%
                         select(idx, month_to_outcome, ever_event, Pde5i)%>%
                         unique())
ggsurvplot(fit, data = surv_dat, censor = T, palette = "jco", pval = F, pval.coord = c(12, 0.8))
#ace_arb <- survfit(Surv(month_to_outcome, ever_event) ~ ace_arb, data = surv_dat)

fit <- survival::coxph(Surv(month_to_outcome, ever_event) ~ Pde5i,
                       data = surv_dat%>%
                         select(idx, month_to_outcome, ever_event, Pde5i)%>%
                         unique())

fit_td <- survival::coxph(Surv(nodes_bl, nodes, status) ~ d_pde5i, 
                          data = surv_dat%>%
                            select(idx, nodes_bl, nodes, status, d_pde5i, month_to_gib_or_death, month_to_outcome)%>%
                            mutate(month_to_gib_or_death = ifelse(is.na(month_to_gib_or_death),
                                                                  month_to_outcome, month_to_gib_or_death),
                                   d_pde5i = ifelse(d_pde5i != 0, 1, 0))%>%
                            filter(nodes <= month_to_gib_or_death))
```
